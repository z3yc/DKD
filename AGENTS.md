...
| dkd-agent | pytest 覆盖率 ≥70%（工具层/状态机/校验器优先）；LLM 调用一律 Fake/打桩，**测试不得真实调用外部 API**；**数据库通道同样不许**——留痕/计量等默认写入器必须依赖注入假实现（`tests/conftest.py` 兜底关闭 `DKD_AGENT_AUDIT_ENABLED`），否则“跑个单测”就会写进本机开发库 |

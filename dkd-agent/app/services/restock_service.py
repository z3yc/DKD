...
class RestockService:
    ...
    async def confirm(self, plan_id: int, *, plan_date: date, user_id: int) -> dict[str, Any]:
        ...
        return await self._execute(plan, decision, plan_id=plan_id, task_id=row.get("task_id"))
    ...
    async def set_pause(self, *, paused: bool, user_id: int, reason: str) -> PauseState:
        ...
        return await self._pause.set_paused(paused=paused, by=str(user_id), reason=reason)

...
public class EmpController extends BaseController
{
    ...
    {
        //根据innerCode查询设备信息
        VendingMachine vendingMachine = vendinngService.selectVendingMachineByInnerCode(innerCode);
        //根据区域id、角色编号、员工状态查询人员列表
        if (vendingMachine == null){
            return error("设备不存在");
        }
        Emp emp = new Emp();
        emp.setRegionId(vendingMachine.getRegionId());//区域id
        emp.setRoleCode(DkdContants.ROLE_CODE_BUSINESS);//角色编号

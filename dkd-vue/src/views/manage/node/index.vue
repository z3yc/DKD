...
<el-table v-loading="loading" :data="nodeList" @selection-change="handleSelectionChange">
  ...
  <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
    <template #default="scope">
      <el-button link type="primary" icon="Edit" @click="getNodeInfo(scope.row)" v-hasPermi="['manage:vm:list']">查看详情</el-button>
<el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['manage:node:edit']">修改</el-button>
      <el-button link type="warning" icon="Cpu" @click="handleAiDiagnose(scope.row)">AI 诊断</el-button>
    ...
    <!-- AI 诊断结果 -->
    <el-dialog title="AI 诊断" v-model="aiDialogOpen" width="600px" append-to-body >
      <div v-loading="aiLoading" element-loading-text="AI 正在分析中..." style="min-height: 200px; padding: 20px;">
        <div v-if="aiAnalysisResult" style="white-space: pre-wrap; line-height: 1.6;">
          {{ aiAnalysisResult }}
        </div>
...
import { getNodeAnalysis } from "@/api/manage/nodeAi";
import { listNode, getNode, delNode, addNode, updateNode, getImportTemplate } from "@/api/manage/node";
...
function handleAiDiagnose(row) {
  ...
  getNodeAnalysis(row.id).then(response => {
    aiAnalysisResult.value = response.data; // 假设返回的 data 是字符串分析结果
...
const upload = reactive({
  ...
  // 上传的地址
  url: import.meta.env.VITE_APP_BASE_API + "/manage/node/import"
});
...
function importTemplate() {
  getImportTemplate().then(response => {
    ...
    const fileName = `node_template_${new Date().getTime()}.xlsx`;

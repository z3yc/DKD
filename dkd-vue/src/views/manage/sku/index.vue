<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="skuList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="序号" type="index" align="center" width="50" prop="skuId" />
      ...
      <el-form ref="skuRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="商品名称" prop="skuName">
         ...
         <el-form-item label="品牌" prop="brandName">
          ...
          <el-input-number v-model="form.price" :min="0.01"  :max="9999.99" :precision="2" :step="0.5"  placeholder="请输入商品价格" />元
        </el-form-item>
    ...
    <el-upload
      ...
      <template v-slot:tip>
        <div class="el-upload__tip">只能上传xls/xlsx格式，且不超过1M</div>
      </template>
    </el-upload>
</el-dialog>
...
function handleExport() {
  ...
  if (ids.value && ids.value.length > 0) {
    // 将数组转换为逗号分隔的字符串，这样后端能正确识别
    exportParams.skuIds = ids.value.join(',');
  }
  
  proxy.download('manage/sku/export', exportParams, `sku_${new Date().getTime()}.xlsx`)
}
/** 打开导入对话框 */
...
// 上传地址
const uploadExcelUrl = ref(import.meta.env.VITE_APP_BASE_API + "/manage/sku/import"); // 上传的excel文件服务器地址
// 上传请求头
...
function handleBeforeUpload(file) {
  ...
  if (!isExcel) {
    proxy.$modal.msgError(
      `文件格式不正确, 请上传${props.fileType.join("/")}excel格式文件!`
    );
    return false;
  }
  if (props.fileSize) {
